...
var fill_database = false;


window.setInterval(function(){
	
	$.getJSON( "GetCentroids.php", function( data ) {
		
		
		k_max=0;
		for(var i = 0; i < data.length;i++){
			if(data[i]["MpiResult_K"] > k_max) k_max = data[i]["MpiResult_K"];
		}
		console.log("K_max = " + k_max);
    ...
    */
    ...
    var painter = {
        ...
        redraw: function() {
            ...
            var ctx = this._activeCanvas.context;
			// Voile du bateau
			...
			if(centroids != undefined )
			{
				...
				for(var i = 0 ; i < centroids.length; i++){
					...
					if( centroids[i]["MpiResult_K"] == k)
					{
						
						ctx.beginPath();
						ctx.rect(centroids[i]["MpiResult_x"],centroids[i]["MpiResult_y"], 10, 10);
